...
const RoadTooltip: React.FC<RoadTooltipProps> = ({ road, position, onClose, onReroll }) => {
  ...
  // Mobile: bottom sheet style, Desktop: positioned tooltip
  const isMobile = window.innerWidth < 768;

  if (isMobile) {
    return (
      <div className="fixed inset-0 z-40 flex items-end md:hidden fade-in">
        <div
          className="w-full bg-mork-black text-mork-yellow border-t-6 border-mork-yellow p-6 max-h-[80vh] overflow-y-auto rounded-t-2xl"
          style={{
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="mork-button w-full text-sm py-2 px-3 flex items-center justify-center gap-1"
            >
              <RefreshCw size={14} />
              Reroll
            </button>
            <button
              onClick={onClose}
              className="mork-button w-full text-sm py-2 px-3"
            >
              Close
            </button>
          </div>
  ...
  return (
    <div
      ref={tooltipRef}
      className="fixed z-40 mork-modal p-6 max-w-lg fade-in hidden md:block"
      style={{
