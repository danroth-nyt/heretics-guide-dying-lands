...
const LocationModal: React.FC<LocationModalProps> = ({ node, onClose, onReroll }) => {
  ...
  return (
    ...
    >
      <div className="mork-modal max-w-2xl w-full mx-4 p-8 max-h-[90vh] overflow-y-auto md:rounded-lg md:max-h-[90vh] h-full md:h-auto">
        {/* Header */}
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-3xl md:text-4xl font-pirata mb-2">{node.locationType}</h2>
          ...
          <button
            onClick={onClose}
            className="p-2 min-h-11 min-w-11 flex items-center justify-center hover:bg-mork-yellow hover:text-mork-black transition-colors"
            aria-label="Close"
          >
            <X size={24} />
          </button>
        ...
        {/* Actions */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 border-t-2 border-mork-yellow">
          <button
            onClick={() => onReroll(node)}
            className="mork-button flex items-center justify-center gap-2 w-full sm:w-auto"
          >
            <RefreshCw size={18} />
            Reroll Details
          </button>
          <button
            onClick={onClose}
            className="mork-button w-full sm:w-auto"
          >
            Close
          </button>
        </div>
